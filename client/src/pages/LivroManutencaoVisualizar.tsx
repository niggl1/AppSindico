...
interface LivroManutencaoVisualizarProps {
  params?: { token: string };
  token?: string;
}

export default function LivroManutencaoVisualizar({ params, token: propToken }: LivroManutencaoVisualizarProps) {
  const token = propToken || params?.token || '';
  const [copied, setCopied] = useState(false);

  // Buscar timeline pelo token
  const { data: timeline, isLoading, error } = trpc.timelineLivro.obterPorToken.useQuery(
    { token },
    { enabled: !!token }
  );

  // Mutation para registar visualização
  const registarVisualizacaoMutation = trpc.timelineLivro.registarVisualizacao.useMutation();
